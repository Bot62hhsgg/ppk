...
const DONATE_LINK = "https://t.me/NgInjEct";
...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    // Build pagination
    document.addPageButton("Prev", `/nz/${page > 0 ? page - 1 : 0}`, page > 0 ? false : true);
    document.addPageButton("Next", `/nz/${page + 1}`, page < Math.floor(proxyList.length / 10) ? false : true);
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/nz")) {
        const page = url.pathname.match(/^\/nz\/(\d+)$/);
...
<html lang="en" id="html" class="scroll-auto scrollbar-hide dark">
  ...
  <body class="bg-white dark:bg-neutral-800 bg-fixed">
    ...
    <div id="container-header">
      <div id="container-info" class="bg-fuchsia-400 border-2 border-neutral-800 text-right px-5">
          ...
          <div class="basis-1/6 w-full h-full rounded-md">
            <div class="flex w-full h-full gap-1 justify-between">
              <button
                ...
                class="basis-1/2 p-2 rounded-full bg-fuchsia-400 flex justify-center items-center"
              >
                Clash
              </button>
              <button
          ...
          <div class="basis-1/6 w-full h-full rounded-md">
            <div class="flex w-full h-full gap-1 justify-between">
              ...
              <button
                ...
                class="p-2 rounded-full bg-fuchsia-400 flex justify-center items-center"
              >
    ...
    <footer>
      <div class="fixed bottom-3 right-3 flex flex-col gap-1 z-50">
        ...
        <button onclick="toggleDarkMode()" class="bg-fuchsia-400 rounded-full border-2 border-neutral-800 p-1">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
        ...
        const res = fetch(url).then(async (res) => {
          ...
          if (res.status == 200) {
            ...
            for (const domain of respJson) {
              ...
              domainElement.classList.add("w-full", "bg-fuchsia-400", "rounded-md");
...
class Document {
  ...
  buildProxyGroup() {
    ...
    for (let i = 0; i < this.proxies.length; i++) {
      ...
      proxyGroupElement += `  </div>`;
      proxyGroupElement += `  <div class="rounded py-1 px-2 bg-fuchsia-400 dark:bg-neutral-800 dark:border-2 dark:border-fuchsia-400">`;
      ...
      for (let x = 0; x < proxyData.list.length; x++) {
        ...
        proxyGroupElement += `<button class="bg-indigo-500 dark:bg-neutral-800 dark:border-2 dark:border-indigo-500 rounded p-1 w-full text-white" onclick="copyToClipboard('${proxy}')">${indexName[x]}</button>`;
  ...
  addPageButton(text, link, isDisabled) {
    const pageButton = `<li><button ${
      isDisabled ? "disabled" : ""
    } class="px-3 py-1 bg-fuchsia-400 border-2 border-neutral-800 rounded" onclick=navigateTo('${link}')>${text}</button></li>`;
